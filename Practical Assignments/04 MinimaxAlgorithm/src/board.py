...
class Board:
    ...
    def get_scores(self, board):
        ...
        for row in range(self.__n):
            for col in range(self.__n):
                if board[row][col] == self.black:
                    black_score += 1 + (1.0 ** row) * row
                elif board[row][col] == self.white:
                    white_score += 1 + (1.0 ** (self.__n - row - 1)) * (self.__n - row - 1)
        ...
        scores[self.black] = black_score - white_score
        scores[self.white] = white_score - black_score
        return scores
